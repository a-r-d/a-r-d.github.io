...
| Graph neural networks | Models designed to work w/ graph structured data. Can learn and make predictions based on relationships between data. |
...
| --- | --- | --- |
| Gallium nitride semiconductors | As opposed to silicon it has higher efficiency and can be useful for lower power consumption. | Cree, Efficient Power Conversion, GaN Systems. |
...
| Cerebras | AI chips | They make full wafer sized chips, which is a unique approach that is well suited for large AI models. They are publishing benchmarks on llama-3.2 that are insanely fast. | They plan to go public soon but are held up by CFIUS. |
...
| Helion Energy | Fusion | Approach is interesting because they are building a solution to directly capture electromagnetic energy so you don’t need a steam plant to get electricity from it.  | It's private, their newest machine should have some interesting results. Sam Altman invested. Microsoft invested. Polaris prototype eta is 2024 so it's behind schedule now. |
...
| Rippling | Payroll, HR, Ops in a box. It's a SaaS that covers a lot of ground for companies and makes life easier. |
